		...
		while (StairsSetup.amountOfStairs > stairNr){
			...
			if (PanelSetup.createPanel){
				...
				if (PanelSetup.panelRigidbody){
					...
					stairPanels.GetComponent<Rigidbody>().angularDrag = 0f;

					if (PanelSetup.panelGravity){
						
						stairPanels.GetComponent<Rigidbody>().useGravity = true;
						
					} else if (!PanelSetup.panelGravity){
						
						stairPanels.GetComponent<Rigidbody>().useGravity = false;
					}
					
					if (PanelSetup.panelKinematic){
						
						stairPanels.GetComponent<Rigidbody>().isKinematic = true;
						
					} else if (!PanelSetup.panelKinematic){
						
						stairPanels.GetComponent<Rigidbody>().isKinematic = false;
					}
				}
				
				if (PanelSetup.createPanel && PanelSetup.stairPanelMaterial != null){
					
					stairPanels.GetComponent<Renderer>().material = PanelSetup.stairPanelMaterial;
				}
